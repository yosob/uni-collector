...
| 组件 | 阶段 | 说明 |
|------|------|------|
| 批量首次深爬 | Phase 3 | 对 22 所院校运行 site-explorer |
...
| `data-organizer` | 保存数据、校验、初始化 | `skills/data-organizer/SKILL.md` |

## 数据采集策略

### 核心：探索 → 固化 → 更新

1. **首次探索** (site-explorer): LLM 从根 URL 自由探索，产出 site_map.md + university_profile.md + 填充数据
2. **日常更新** (smart-extractor): 按 site_map 中的已知 URL 提取，成本低
3. **定期刷新**: 申请季前（4/11月）集中 + 平时错峰（每天 1-2 所，按列表顺序）
...
TODO: 当前由 LLM 自行判断。需要实践数据后设计更精确的停止条件（字段填满率 / 页面类型覆盖 / token 预算）。
