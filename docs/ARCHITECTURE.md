...
- **入口**: 院校根 URL
- **方式**: LLM 自由浏览网站，按 exploration-guide 引导
- **产出**: `site_map.md`（URL 架构）+ `university_profile.md`（信息摘要）+ 填充 `_index.md`
- **成本**: 高（LLM 探索多个页面）
- **策略**: **专业穷举** — 找到专业汇总页（Übersicht der Studiengänge）后，提取全部专业列表，逐一访问每个专业及其子页面，不遗漏
- **页面预算**: 按专业数量合理预估（每个专业 3-5 页 + 共享页面 10-15 页）
- **停止条件**: 专业汇总页中所有专业都已访问，且每个专业 P0 字段已提取
...
4. **逐一访问每个专业** — 每个专业至少访问概述页 + 申请页 + 课程页
