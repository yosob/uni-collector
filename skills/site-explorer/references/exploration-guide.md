...
5. **递归探索子页面**：访问发现的子页面，检查是否有进一步的子页面，递归直到 LLM 判断当前页面没有新的有价值子页面
...
- **不遗漏**: 汇总页中每个专业链接都必须记录
- **记录子页面类型**: 每个发现的 URL 标注页面类型（申请、课程、教授等）
- **递归深入**: 发现子页面后访问它，检查是否有进一步的有价值子页面，由 LLM 判断何时停止
- **记录来源**: 标注每个 URL 是从哪个页面发现的

### 边界判断

**绝对不能停止探索的情况**：
- 专业汇总页中还有未访问的专业链接
- 某个子页面中还有未访问的有价值子链接（申请、课程、教授、作品集等）

**可以停止探索的情况**：
- 专业汇总页中所有专业都已递归探索完毕
- 当前页面的所有链接都已记录或指向无关内容
...
**注意**：site-explorer 只负责发现 URL 和生成 sitemap，不提取数据、不生成 profile。数据提取由 smart-extractor 负责，profile 生成由 data-organizer 负责。
