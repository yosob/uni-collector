...
LLM 驱动的网站发现，从院校根 URL 出发，发现设计/艺术专业相关的所有页面 URL。**核心要求：发现专业汇总页后，必须逐一访问每个专业概述页，发现所有子页面 URL，不遗漏。**
...
**目的**：发现子页面链接，不提取数据。浏览页面内容，识别所有相关子页面链接。
...
记录每个子页面的 URL 和页面类型。**不需要访问子页面本身**，只需发现并记录 URL。

#### c) 标记已完成

发现一个专业的所有子页面 URL 后在清单中标记为 ✓。
...
本 skill 只访问到专业概述页（不深入子页面），页面数量远少于完整探索：
- 每个专业 1 页（概述页，用于发现子页面链接）
- 共享页面 5-10 个（首页、汇总页、导航页、申请总入口）
- 例如 10 个专业的院校约需 15-20 个页面
